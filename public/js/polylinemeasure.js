(function (factory) {
    ...
    L.Control.PolylineMeasure = L.Control.extend({
        ...
        options: {
            ...
            tooltipTextDelete:
                "Press SHIFT-key and click to <b>delete point</b>",
            tooltipTextMove: "Click and drag to <b>move point</b><br>",
            tooltipTextResume:
                "<br>Press CTRL-key and click to <b>resume line</b>",
        ...
        _toggleMeasure: function () {
            ...
            if (this._measuring) {
                ...
                this._measureControl.title =
                    this.options.measureControlTitleOff;
                ...
                this._map.off(
                    "mousemove",
                    this._resumeFirstpointMousemove,
                    this
                );
        ...
        _computeDistance: function (line) {
            ...
            line.circleCoords.map(
                function (point, point_index) {
                    if (point_index >= 1) {
                        var distance = line.circleCoords[
                        point_index - 1
                            ].distanceTo(line.circleCoords[point_index]);
        ...
        _onKeyDown: function (e) {
            if (e.keyCode === 27) {
                ...
                if (this._resumeFirstpointFlag === true) {
                    ...
                    /* text =
                        text +
                        '<div class="polyline-measure-tooltip-difference">+' +
                        "0</div>"; */
                    text =
                        text +
                        '<div class="polyline-measure-tooltip-total">' +
                        "0</div>";
                    this._arrPolylines[lineNr].tooltips[0]._icon.innerHTML =
                        text;
                    this._arrPolylines[lineNr].tooltips.map(
                        function (item, index) {
                            if (index >= 1) {
                                var distance = this._arrPolylines[
                                    lineNr
                                    ].circleCoords[index - 1].distanceTo(
                                    this._arrPolylines[lineNr].circleCoords[
                                        index
                                        ]
                                );
        ...
        _drawArrow: function (arcLine) {
            ...
            var iconArrow = L.divIcon({
                ...
                html:
                    ...
                    "; font-size: 16px; line-height: 16px; vertical-align:top; transform: rotate(" +
                    cssAngle +
                    "deg)'>&#x27a4;</div>", // best results if iconSize = font-size = line-height and iconAnchor font-size/2 .both values needed to position symbol in center of L.divIcon for all font-sizes.
            });
        ...
        _startLine: function (clickCoords) {
            ...
            this._currentLine = {
                    ...
                    if (lastCircleMarker) {
                        ...
                        if (this.circleMarkers.length === 1) {
                            lastCircleMarker.setStyle(
                                polylineState.options.startCircle
                            );
                        } else {
                            lastCircleMarker.setStyle(
                                polylineState.options.intermedCircle
                            );
                        }
                    }
                ...
                addPoint: function (mouseCoords) {
                    ...
                    if (
                        lastCircleCoords &&
                        lastCircleCoords.equals(mouseCoords)
                    ) {
                    ...
                    if (this.circleCoords.length > 1) {
                        ...
                        var distanceSegment =
                            lastCircleCoords.distanceTo(mouseCoords);
                        this.distance += distanceSegment;
                        var currentTooltip =
                            polylineState._currentLine.tooltips.last();
                        var prevTooltip =
                            polylineState._currentLine.tooltips.slice(
                                -1,
                                -2
                            )[0];
                ...
                finalize: function () {
                    ...
                    if (this.circleCoords.length > 1) {
                        ...
                        ); // add Class e.g. another background-color to the Previous Tooltip (which is the last fixed tooltip, cause the moving tooltip is being deleted later)
                        var lastCircleMarker = this.circleMarkers.last();
                        lastCircleMarker.setStyle(
                            polylineState.options.endCircle
                        );
                        ...
                        polylineState._currentLine.circleMarkers.map(function (
                            circle
                        ) {
                            circle.bindTooltip(
                        ...
                        polylineState._currentLine.arrowMarkers.map(function (
                            arrow
                        ) {
                            arrow.bindTooltip(
                                polylineState.options.tooltipTextAdd,
                                {
                                    direction: "top",
                                    opacity: 0.7,
                                    className: "polyline-measure-popupTooltip",
        ...
        _clickedArrow: function (e) {
            if (e.originalEvent.ctrlKey) {
                ...
                this._arrPolylines[lineNr].circleMarkers.map(function (
                    item,
                    index
                ) {
                ...
                lineCoords =
                    this._arrPolylines[lineNr].polylinePath.getLatLngs(); // get Coords of each Point of the current Polyline
                ...
                Array.prototype.splice.apply(
                    lineCoords,
                    [arrowNr * (this._arcpoints - 1), this._arcpoints].concat(
                        arc1,
                        arc2
                    )
                );
                ...
                this._arrPolylines[lineNr].arrowMarkers.map(function (
                    item,
                    index
                ) {
                    item.cntLine = lineNr;
        ...
        _dragCircleMousemove: function (e2) {
            ...
            if (circleNr >= 1) {
                ...
                Array.prototype.splice.apply(
                    lineCoords,
                    [(circleNr - 1) * (arcpoints - 1), arcpoints].concat(
                        newLineSegment1
                    )
                );
                ...
                this._arrPolylines[lineNr].arrowMarkers[circleNr - 1] =
                    arrowMarker;
            }
            if (circleNr < this._arrPolylines[lineNr].circleCoords.length - 1) {
                ...
                Array.prototype.splice.apply(
                    lineCoords,
                    [circleNr * (arcpoints - 1), arcpoints].concat(
                        newLineSegment2
                    )
                );
        ...
        _dragCircle: function (e1) {
            ...
            if (e1.originalEvent.shiftKey) {
                ...
                if (this._arrPolylines[lineNr]) {
                    ...
                    var lineCoords =
                        this._arrPolylines[lineNr].polylinePath.getLatLngs();
                    ...
                    if (circleNr === 0) {
                        ...
                        circleNr ===
                        this._arrPolylines[lineNr].circleCoords.length
                    ) {
                        this._arrPolylines[lineNr].circleMarkers[
                        circleNr - 1
                            ].on("click", this._resumePolylinePath, this);
                        this._arrPolylines[lineNr].circleMarkers[
                        circleNr - 1
                            ].bindTooltip(
                            this.options.tooltipTextMove +
                        ...
                        Array.prototype.splice.apply(
                            lineCoords,
                            [
                                (circleNr - 1) * (arcpoints - 1),
                                2 * arcpoints - 1,
                            ].concat(newLineSegment)
                        );
                        this._arrPolylines[lineNr].arrowMarkers[
                        circleNr - 1
                            ].removeFrom(this._layerPaint);
                    ...
                    this._arrPolylines[lineNr].tooltips.map(
                        function (item, index) {
                            if (index >= 1) {
                                ...
                                totalDistance += distance;
                                this._arrPolylines[lineNr].distance =
                                    totalDistance;
                                var prevTooltip =
                                    this._arrPolylines[lineNr].tooltips[
                                    index - 1
                    ...
                    this._currentLine.tooltips[circleNr].removeFrom(
                        this._layerPaint
                    );
                    ...
                    if (circleNr === 0) {
                        ...
                        circleNr === this._currentLine.circleCoords.length
                    ) {
                        ...
                        this._currentLine.arrowMarkers.splice(
                            circleNr - 1,
                            2,
                            arrowMarker
                        );
                    ...
                    this._currentLine.tooltips.map(
                        function (item, index, arr) {
                            if (index >= 1) {
                                ...
                                var prevTooltip =
                                    this._currentLine.tooltips[index - 1];
                                var lastCircleCoords =
                                ...
                                if (index === arr.length - 1) {
                                    ...
                                    index - 1
                                        ...
                                        this._currentLine.circleCoords[index]
                                    );
                                    mouseCoords =
                                        this._currentLine.circleCoords[index];
...
var options = {
    ...
    measureControlLabel: '<img src="/images/icon/measuretape.svg" alt="" />',
    ...
    clearControlLabel: '<img src="/images/icon/stop.svg" alt="" />', // Label of the Clear Control (Unicode symbols are possible)
    ...
    tempLine: {
        ...
        color: "#ffff00", // Dashed line color
        weight: 2, // Dashed line weight
    },
    fixedLine: {
        // Styling for the solid line
        color: "#ffff00", // Solid line color
        weight: 2, // Solid line weight
    },
    arrow: {
        // Styling of the midway arrow
        color: "#ffff00", // Color of the arrow
    },
    startCircle: {
        // Style settings for circle marker indicating the starting point of the polyline
