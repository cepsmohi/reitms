...
(function () {
    ...
    let _merge = function (object1, object2) {
    ...
    let _defaultMarker = function () {
        //Generate a default marker
        return L.marker(null, {
            icon: L.divIcon({
                html: '<div><img style="width: 20px;" src="../images/icon/gps.svg" alt="" /></div>',
                iconAnchor: [10, 10],
                className: "",
            }),
            attribution: "",
    ...
    L.Geolet = L.Control.extend({
        options: {
            ...
            className: "geoleticon",
            activeClassName: null,
            style: {display: "flex", color: ""},
            activeStyle: {display: "flex", color: "#E00"},
            html: '<img style="width: 100%;" src="/images/icon/geolet.svg" alt="" />',
            geoOptions: {
        ...
        activate: function () {
            ...
            let geoSuccessCallback = function (data) {
                let first = !!control._first;
            ...
            let geoErrorCallback = function (data) {
                control.deactivate();
            ...
            let dir =
                key === "lat"
                    ...
                    : latLng[key] < 0
                        ? symbols.W
                        : symbols.E;
            let val = Math.round(Math.abs(latLng[key]) * 3600) / 3600;
            let deg = Math.floor(val) + symbols.deg;
            let min = (Math.floor(val * 60) % 60) + symbols.min;
            let sec = (Math.floor(val * 3600) % 60) + symbols.sec;
