L.Control.BoxZoom = L.Control.extend({
	...
	onAdd: function (o) {
		...
		return (
			this.options.aspectRatio &&
				...
				(this.map.boxZoom._onMouseMove =
					this._boxZoomControlOverride_onMouseMove),
				(this.map.boxZoom._onMouseUp =
					this._boxZoomControlOverride_onMouseUp)),
			this.controlDiv
		);
	},
	onRemove: function () {
		this.options.aspectRatio &&
			...
			(this.map.boxZoom._onMouseMove =
				L.Map.BoxZoom.prototype._onMouseMove),
	...
	_boxZoomControlOverride_onMouseMove: function (o) {
		...
		this.aspectRatio && (l = a / this.aspectRatio),
			(i.style.width = a + "px"),
			(i.style.height = l + "px");
	},
	_boxZoomControlOverride_onMouseUp: function () {
		let t = this._box._leaflet_pos,
			i = new L.Point(
				this._box._leaflet_pos.x + this._box.offsetWidth,
				this._box._leaflet_pos.y + this._box.offsetHeight
			),
